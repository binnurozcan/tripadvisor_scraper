...
def yorumlarial(soup):
    ...
    #baslik = (item.find('span', {'class': 'noQuotes'}).text.strip())
    try:
        for item in yorum:
...
for x in range(0,15800,15):
        soup = get_soup(f'https://www.tripadvisor.co.uk/Restaurant_Review-g186338-d4495284-Reviews-or{x}-Aqua_Shard-London_England.html')
        yorumlarial(soup)
